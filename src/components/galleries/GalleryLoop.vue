<template>
  <ul role="list"
    ...
    <li v-for="g in gallery" :key="g.name">
      <a href="#" class="space-y-4 space-x-2 rounded-md block bg-fuchsia-600 bg-opacity-20 hover:bg-opacity-40 pt-2">
        <img class="h-20 w-20 ml-2 rounded-md lg:h-24 lg:w-24" :src="g.imageUrl" :alt=g.name />
        <div
          ...
          <p class="tinyText pr-3 pt-1">{{ g.blurb }}</p>
        </div>
      </a>
    </li>
  </ul>
